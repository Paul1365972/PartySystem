			...
			public void run() {
				try {
					for (;;) {
						...
						if (operation == RESOURCEPACK) {
							UUID uuid = new UUID(passdis.readLong(), passdis.readLong());
							Functions.setResourcePack(uuid, passdis.readUTF(), RequestClient.this);
						} else {
